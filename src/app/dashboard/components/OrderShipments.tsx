...
import { VStack, Box, Text, Alert, AlertIcon } from "@chakra-ui/react";
    ...
    <VStack align="stretch" spacing={4}>
      {shipments.map((shipment, index) => (
        <Box key={index} p={4} borderWidth={1} borderRadius="md">
          <Text>Order Date: {format(shipment.orderDate, "MMM d, yyyy")}</Text>
          <Text>Order Quantity: {shipment.orderQuantity}</Text>
          <Text>Ship Date: {format(shipment.shipDate, "MMM d, yyyy")}</Text>
          <Text>Ship Quantity: {shipment.shipQuantity}</Text>
          <Text>
            Required Arrival Date:{" "}
            {format(shipment.requiredArrivalDate, "MMM d, yyyy")}
          </Text>
